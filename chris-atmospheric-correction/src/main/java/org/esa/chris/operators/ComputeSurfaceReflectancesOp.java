...
import org.esa.chris.operators.lut.ModtranLookupTable;
import org.esa.chris.operators.lut.ModtranLookupTableReader;
import org.esa.chris.operators.lut.RtcTable;
import org.esa.chris.util.OpUtils;
import org.esa.chris.util.math.internal.LegendrePolynomials;
...
import org.esa.chris.util.math.internal.Regression;
import org.esa.chris.util.math.internal.Roots;
import org.esa.chris.util.math.internal.SimpleLinearRegression;
...
public class ComputeSurfaceReflectancesOp extends Operator {
    ...
    private void initialize2(Rectangle targetRectangle, ProgressMonitor pm) {
        ...
        if (cwvIni == 0.0) {
            ...
            cwvIni = (cwvMax - cwvMin) * Math.sin(day / 365.0) + cwvMin;
        }
        ...
        if (mode == 1 || mode == 5) {
            final SmileCorrectionCalculator scc = SmileCorrectionCalculator.create(true);
    ...
    private class Ac1 implements Ac {
        ...
        private void performSpectralPolishing(Map<Band, Tile> targetTileMap, Rectangle targetRectangle,
            ...
            try {
                ...
                final Band redBand = OpUtils.findBand(rhoBands, band -> band.getSpectralWavelength() >= 670.0);
                final Band nirBand = OpUtils.findBand(rhoBands, band -> band.getSpectralWavelength() >= 785.0);

                final Tile redTile = targetTileMap.get(redBand);
                final Tile nirTile = targetTileMap.get(nirBand);
                final List<SpikyPixel> spikyPixelList = new ArrayList<>();
                ...
                if (spikyPixelList.size() > SPIKY_PIXEL_COUNT) {
                    ...
                    final int lowerRed = OpUtils.findBandIndex(rhoBands, band -> band.getSpectralWavelength() >= 694.7);
                    final int upperRed = OpUtils.findBandIndex(rhoBands, band -> band.getSpectralWavelength() > 772.5);
        ...
        private double wv(final double[] toa, final double[] rho) {
            ...
            final UnivariateFunction function = cwv -> {
                final Calculator calculator1 = calculatorFactory.createCalculator(cwv);
                calculator1.calculateToaRadiances(rho, sim, upperWva + 1, upperWvb + 1);
    ...
    static double[][] readEndmemberTable() throws OperatorException {
        ...
        try (ImageInputStream iis = OpUtils.getResourceAsImageInputStream(ComputeSurfaceReflectancesOp.class,
                                                                          "endmembers.img")) {
